...
if is_train:
    ...
    infer_params["n_estimators"] = int(8000)
    infer_lgb_model = lgb.LGBMRegressor(**infer_params)
    infer_lgb_model.fit(df_train_feats[feature_name], df_train_target,eval_set=[(df_train_feats[feature_name], df_train_target)],
                    callbacks=[
                        lgb.callback.log_evaluation(period=100),
                    ],)
    os.system('mkdir models_'+str(len(feature_name)))
    joblib.dump(infer_lgb_model, './models_'+str(len(feature_name))+'/lgbFull.model')
    if is_offline:   
        # offline predictions
