...
def imbalance_features(df):
    ...
    # V3
    for col in ['matched_size', 'imbalance_size', 'reference_price',"matched_imbalance",'imbalance_buy_sell_flag']:
        for window in [1, 2, 3, 5, 10]:
            df[f"{col}_shift_{window}"] = df.groupby(['stock_id','date_id'])[col].shift(window)
            
    for col in ['matched_size', 'imbalance_size', 'reference_price',"matched_imbalance"]:
        for window in [1, 2, 3, 5, 10]:
            df[f"{col}_ret_{window}"] = df.groupby(['stock_id','date_id'])[col].pct_change(window)
    
    for col in ['ask_price', 'bid_price',
                'wap', 'near_price', 'far_price','market_urgency']:
        for window in [1, 2, 3, 5, 10]:
...
def generate_all_features(df):
    ...
    feature_name = [i for i in df.columns if i not in ["stock_weights","row_id", "target", "time_id", "date_id","far_price_diff_2","matched_imbalance_ret_2","matched_imbalance_ret_3","bid_size_ask_size_imbalance_size_imb2","imbalance_buy_sell_flag_shift_5"]]
...
df_offline_train = df_offline_train.fillna(0)
df_offline_valid = df_offline_valid.fillna(0)
...
model.to(DEVICE)
optimizer = torch.optim.SGD(model.parameters(), momentum=0.9, weight_decay=5e-4,lr=LEARNING_RATE)
...
torch.save(model.state_dict(), "test_lgd.pth")