...
def imbalance_features(df):
    ...
    df['median_imb_new_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['imb_new'].transform('median')
    df['mean_imb_new_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['imb_new'].transform('mean')
    ...
    df['median_matched_size_new_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['matched_size_new'].transform('median')
    ...
    df['weighted_urgency'] = df['market_urgency'] * df['stock_weights']
    ...
    # V3
    for col in ['matched_size_new', 'imb_new','wap','weighted_wap','matched_imbalance_3','imbalance_buy_sell_flag', 'market_urgency']:
        for window in [1, 2, 3, 5, 10]:
            df[f"{col}_shift_{window}"] = df.groupby(['stock_id','date_id'])[col].shift(window)
    df["liquidity_imbalance_shift_1"] = df.groupby(['stock_id','date_id'])["liquidity_imbalance"].shift(1)
    df["liquidity_imbalance_shift_2"] = df.groupby(['stock_id','date_id'])["liquidity_imbalance"].shift(2)
    
    for col in ['wap', 'market_urgency', 'ask_size','bid_size']:
        for window in [1, 2, 3, 5, 10]:
            df[f"{col}_ret_{window}"] = df.groupby(['stock_id','date_id'])[col].pct_change(window)
    
    for col in ["wap", 'market_urgency','liquidity_imbalance',
                'ask_size','bid_size','matched_size_new','matched_imbalance_3',
                'imb_new','imbalance_buy_sell_flag']:
        for window in [1, 2, 3, 5, 10]:
            df[f"{col}_diff_{window}"] = df.groupby("stock_id")[col].diff(window)
            
    
    df['median_weighted_urgency_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["weighted_urgency"].transform('median') 
    df['min_weighted_urgency_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["weighted_urgency"].transform('mean') 
    df['mean_weighted_urgency_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["weighted_urgency"].transform('min') 
    ...
    df["range_weighted_wap_t"] = df["max_weighted_wap_t"] - df["min_weighted_wap_t"]
...
def generate_all_features(df):
    ...
    feature_name = [i for i in df.columns if i not in ["ask_price_bid_price_imb","ask_price_bid_price_reference_price_imb2","bid_price_wap_reference_price_imb2",
                                                       'matched_size_bid_size_ask_size_imb2',"bid_size_ask_size_imbalance_size_imb2",
                                                       "volume","stock_id","stock_weights","row_id", "target", "time_id", "date_id","price_spread",
                                                       "spread_intensity","all_prices_kurt","imbalance_buy_sell_flag_shift_10",
                                                      "market_urgency_shift_10"]]
...
if is_train:
    for nl in [1200]:
        for ss in [0.55]:
            for cb in [0.42]:
                ...
                df_train = df[~offline_split]
                print(df_train.shape)
                df_valid = df[offline_split]
                ...
                feature_name = df_train_feats.columns
    ...
    del df_train_feats, df_valid_feats, df_train, df_valid
    gc.collect()
    df_train = df
    global_stock_id_feats = {
        "median_size": df_train.groupby("stock_id")["bid_size"].median() + df_train.groupby("stock_id")["ask_size"].median(),
    ...
    joblib.dump(infer_lgb_model, './models/lgbFull.model')
    if is_offline:   
        # offline predictions
