...
EPOCHS = 500
LEARNING_RATE = 1e-4
model = Model(df_offline_train.shape[1]-3)
DEVICE = torch.device('cuda:1')

model.to(DEVICE)
from pytorch_ranger import Ranger
optimizer = Ranger(model.parameters(), lr=LEARNING_RATE)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(0.25*EPOCHS)*df_offline_train.shape[0])
...
torch.save(model.state_dict(), "test_lstm_simple.pth")