...
def imbalance_features(df):
    ...
    # df["bid_size_diff_1"] = df.groupby(["stock_id","date_id"])["bid_size"].diff(1)
    # df["bid_size_shift_1"] = df.groupby(["stock_id","date_id"])["bid_size"].shift(1)
    
    # df["bid_size_change"] = (df["bid_price_diff_1"] > 0) * df["bid_size"] + (df["bid_price_diff_1"]  == 0) * df["bid_size_diff_1"] +  (df["bid_price_diff_1"] < 0) * (-df["bid_size_shift_1"])
    
    # df["ask_size_diff_1"] = df.groupby(["stock_id","date_id"])["ask_size"].diff(1)
    # df["ask_size_shift_1"] = df.groupby(["stock_id","date_id"])["ask_size"].shift(1)
    # df["ask_size_change"] = (df["ask_price_diff_1"] > 0) * (-df["ask_size_shift_1"]) + (df["ask_price_diff_1"]  == 0) * df["ask_size_diff_1"] + (df["ask_price_diff_1"] < 0) * (df["ask_size"])
    ...
    # df['median_OFI_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["OFI"].transform('median') 
    # df['min_OFI_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["OFI"].transform('min') 
    # df['max_OFI_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["OFI"].transform('max') 
    # df['mean_OFI_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["OFI"].transform('mean')
    # df['median_ask_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["ask_size_change"].transform('median') 
    # df['min_ask_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["ask_size_change"].transform('min') 
    # df['max_ask_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["ask_size_change"].transform('max') 
    # df['mean_ask_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["ask_size_change"].transform('mean')
    # df['median_bid_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["OFI"].transform('median') 
    # df['min_bid_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["bid_size_change"].transform('min') 
    # df['max_bid_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["bid_size_change"].transform('max') 
    # df['mean_bid_size_change_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["bid_size_change"].transform('mean')
    # df['OFI_rolling_mean5'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=5).mean().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_mean10'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=10).mean().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_mean15'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=15).mean().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_std5'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=5).std().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_std10'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=10).std().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_std15'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=15).std().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_ptp5'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=5).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=5).min().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_ptp10'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=10).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=10).min().reset_index(level=(0,1), drop=True).sort_index()
    # df['OFI_rolling_ptp15'] = df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=15).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['OFI'].rolling(window=15).min().reset_index(level=(0,1), drop=True).sort_index()
    ...
    #df['min_weighted_wap_t'] = df.groupby(['seconds_in_bucket', 'date_id'])["weighted_wap"].transform('min') 
...
def generate_all_features(df):
    ...
    feature_name = [i for i in df.columns if i not in ["ask_size_diff_1","ask_size_shift_1","bid_size_diff_1","bid_size_shift_1","imbalance_buy_sell_flag","stock_weights","row_id", "target", "time_id", "date_id","far_price_diff_2","matched_imbalance_ret_2","matched_imbalance_ret_3","bid_size_ask_size_imbalance_size_imb2","imbalance_buy_sell_flag_shift_5"]]
...
if is_train:
    ...
    for nl in [1200]:
        for ss in [0.55]:
            for cb in [0.45]:
                ...
                os.system('mkdir models_111'+str(len(feature_name)))
                joblib.dump(lgb_model, './models_111'+str(len(feature_name))+'/lgbPart.model')
    ...
    infer_params["n_estimators"] = int(1.5 * lgb_model.best_iteration_)
    infer_lgb_model = lgb.LGBMRegressor(**infer_params)
    infer_lgb_model.fit(df_train_feats[feature_name], df_train_target)
    os.system('mkdir models_new111'+str(len(feature_name)))
    joblib.dump(infer_lgb_model, './models_new111'+str(len(feature_name))+'/lgbFull.model')
    if is_offline:   
        # offline predictions
