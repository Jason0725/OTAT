...
def imbalance_features(df):
    ...
    df['imb_vs_market'] = df["size_imbalance"] - df['mean_imb_t']
    ...
    for col in ['ask_price', 'bid_price',
                'wap', 'near_price', 'far_price',"reference_price_wap_imb","market_urgency","imb_new","price_spread"]:
        for window in [1, 2, 3, 5, 10]:
            df[f"{col}_diff_{window}"] = df.groupby(['stock_id','date_id'])[col].diff(window)
            
            
    df["trading_volume"] = df.groupby(['stock_id','date_id'])["matched_size"].diff()
    df['trading_volume_mean5'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=5).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_mean10'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=10).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_mean15'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=15).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_std5'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=5).std().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_std10'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=10).std().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_std15'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=15).std().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_ptp5'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=5).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=5).min().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_ptp10'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=10).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=10).min().reset_index(level=(0,1), drop=True).sort_index()
    df['trading_volume_ptp15'] = df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=15).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['trading_volume'].rolling(window=15).min().reset_index(level=(0,1), drop=True).sort_index()
    df['median_trading_volume_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['trading_volume'].transform('median') 
    df['std_trading_volume_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['trading_volume'].transform('std') 
    df['max_trading_volume_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['trading_volume'].transform('max') 
    df['min_trading_volume_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['trading_volume'].transform('min') 
    df['mean_trading_volume_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['trading_volume'].transform('mean')
    df["range_trading_volume_t"] = df["max_trading_volume_t"] - df["min_trading_volume_t"]
    
    df["trading_volume_vs_self"] = df["trading_volume"] - df['trading_volume_mean15']
    ...
    for col in ['mean_weighted_wap_t']:
        for window in [1, 2, 3, 5, 10]:
            df[f"{col}_diff_{window}"] = df.groupby(['stock_id','date_id'])[col].diff(window)
    ...
    df["market_urgency_vs_market"] = df["market_urgency"] - df['mean_market_urgency_t']
    ...
    df["reference_price_wap_imb_vs_market"] = df["reference_price_wap_imb"] - df['mean_reference_price_wap_imb_t']
    ...
    df['wap_rolling_mean20'] = df.groupby(['stock_id',"date_id"])['wap'].rolling(window=20).mean().reset_index(level=(0,1), drop=True).sort_index()
    ...
    df["wap_vs_market"] = df["wap"] - df["mean_imb2_t"]
    df["wap_MACD"] = df['wap_rolling_mean20'] - df['wap_rolling_mean5']
    
    df["wap_vs_self"] = df["wap"] - df['wap_rolling_mean15']
    
    
    df["gain"] = np.where(df["wap_diff_1"] > 0,1, 0)
    df["loss"] = np.where(df["wap_diff_1"] < 0,1, 0)

    avg_gain = df.groupby(['stock_id',"date_id"])["gain"].rolling(window=20,min_periods=10).mean().reset_index(level=(0,1), drop=True).sort_index()
    avg_loss = df.groupby(['stock_id',"date_id"])["loss"].rolling(window=20,min_periods=10).mean().reset_index(level=(0,1), drop=True).sort_index()

    rs = avg_gain / avg_loss
    df["rsi"] = 100 - (100 / (1 + rs))
    
        #new
    df["cluster"] =  df["stock_id"].map(clusters)
    df["same_group_mean"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["wap"].transform("mean")
    df["same_group_median"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["wap"].transform("median")
    df["same_group_std"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["wap"].transform("std")
    df["same_group_ptp"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["wap"].transform("max") - df.groupby(["date_id","seconds_in_bucket","cluster"])["wap"].transform("max")
    df["same_trading_volume_mean"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["trading_volume"].transform("mean")
    df["same_trading_volume_median"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["trading_volume"].transform("median")
    df["same_trading_volume_std"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["trading_volume"].transform("std")
    df["same_trading_volume_ptp"] = df.groupby(["date_id","seconds_in_bucket","cluster"])["trading_volume"].transform("max") - df.groupby(["date_id","seconds_in_bucket","cluster"])["trading_volume"].transform("max")

    # for i in range(10):
    #     r = df[df["cluster"]==i].groupby(["date_id","seconds_in_bucket"])["wap"].max().reset_index()
    #     r.columns = ["date_id","seconds_in_bucket",f"max_wap_cluster_{i}"]
    #     df = pd.merge(df, r, on=['date_id', 'seconds_in_bucket'], how='left')
    ...
    df['bid_size_rolling_mean15'] = df.groupby(['stock_id',"date_id"])['bid_size'].rolling(window=15).mean().reset_index(level=(0,1), drop=True).sort_index()
    ...
    df['reference_price_wap_imb_rolling_mean5'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=5).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_mean10'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=10).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_mean15'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=15).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_std5'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=5).std().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_std10'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=10).std().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_std15'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=15).std().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_ptp5'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=5).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=5).min().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_ptp10'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=10).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=10).min().reset_index(level=(0,1), drop=True).sort_index()
    df['reference_price_wap_imb_rolling_ptp15'] = df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=15).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['reference_price_wap_imb'].rolling(window=15).min().reset_index(level=(0,1), drop=True).sort_index()
    
    df["reference_price_wap_imb_vs_self"] = df["reference_price_wap_imb"] - df['reference_price_wap_imb_rolling_mean15']
...
def generate_all_features(df):
    ...
    feature_name = [i for i in df.columns if i not in ["gain","loss","imbalance_buy_sell_flag","stock_weights","row_id", "target", "time_id", "date_id","far_price_diff_2","matched_imbalance_ret_2","matched_imbalance_ret_3","bid_size_ask_size_imbalance_size_imb2","imbalance_buy_sell_flag_shift_5"]]
...
clusters = [ 4,  7,  7,  4,  4, 10,  8,  2,  8,  5,  5, 10,  5,  2,  1,  3,  8,
        2, 10,  6, 10,  3,  1,  1,  5,  9,  5,  2,  9,  8,  5,  6,  5,  3,
        7,  4, 10,  4, 10,  9,  5,  1,  7,  8,  9,  1,  8, 10,  3,  5,  3,
        3,  9,  4,  3,  4,  3, 10,  3, 10,  3, 10,  7,  5,  7,  5, 10, 10,
        4,  8,  7,  7,  1, 10, 10,  1,  9, 10,  8, 10,  7,  7,  7,  1,  1,
        7,  8,  2,  1,  2, 10,  3,  7,  5,  8,  1,  1,  7, 10, 10,  7,  6,
        3,  3,  5,  4,  4,  7,  8,  4,  5,  3,  1,  3,  8, 10,  9,  7,  8,
        7, 10,  9,  5,  5, 10,  6,  1, 10,  5,  7,  4, 10, 10,  4,  7,  3,
       10, 10,  7,  3,  1,  1,  2,  7,  5,  3,  8,  9,  5,  9, 10,  4,  7,
        7,  3,  7, 10,  3,  6,  9,  4, 10,  3,  4,  4, 10,  9, 10,  1,  3,
        5,  5,  7,  8, 10,  1,  3,  7,  3,  1,  7, 10,  5,  1,  3, 10,  4,
        9,  3,  3, 10,  1,  9,  1,  5,  5,  9,  3,  5,  9]

clusters = {int(k):int(v) for k,v in enumerate(clusters)}
...
if is_train:
            ...
            for cb in [0.45]:
                ...
                for i in range(5):
                    ...
                    lgb_model.fit(
                        ...
                        callbacks=[
                            lgb.callback.early_stopping(stopping_rounds=1500),
                            lgb.callback.log_evaluation(period=100),
                            ],
                    )
                    os.system('mkdir models_LGBfivefold'+str(len(feature_name)))
                    joblib.dump(lgb_model, './models_LGBfivefold'+str(len(feature_name))+f'/lgbPart_{i}.model')
    ...
    infer_params["n_estimators"] = int(1.8*lgb_model.best_iteration_)
    infer_lgb_model = lgb.LGBMRegressor(**infer_params)
    infer_lgb_model.fit(df_train_feats[feature_name], df_train_target)
    os.system('mkdir models_LGBfivefold'+str(len(feature_name)))
    joblib.dump(infer_lgb_model, './models_LGBfivefold'+str(len(feature_name))+'/lgbFull.model')
    if is_offline:   
        # offline predictions
