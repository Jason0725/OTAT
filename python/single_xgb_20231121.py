...
def imbalance_features(df):
    ...
    df['urgency_2'] = df["bid_size"]/df["ask_size"] * (df["reference_price"] - df["wap"])
    ...
    for col in ['ask_price', 'bid_price',
                'wap', 'near_price', 'far_price',"reference_price_wap_imb","market_urgency","imb_new","price_spread","matched_imbalance_3","urgency_2"]:
        for window in [1, 2, 3, 5, 10]:
    ...
    df['std_spread_depth_ratio_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['spread_depth_ratio'].transform('std') 
    df['max_spread_depth_ratio_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['spread_depth_ratio'].transform('max') 
    df['min_spread_depth_ratio_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['spread_depth_ratio'].transform('min')
    df["range_spread_depth_ratio_t"] = df["max_spread_depth_ratio_t"] - df["min_spread_depth_ratio_t"]
    
    df['median_price_pressure_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['price_pressure'].transform('median') 
    df['mean_price_pressure_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['price_pressure'].transform('mean')
    
    df['median_matched_imbalance_3_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['matched_imbalance_3'].transform('median') 
    df['mean_matched_imbalance_3_t'] = df.groupby(['seconds_in_bucket', 'date_id'])['matched_imbalance_3'].transform('mean')
    ...
    df['urgency_2_rolling_mean5'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=5).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_mean10'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=10).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_mean15'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=15).mean().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_std5'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=5).std().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_std10'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=10).std().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_std15'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=15).std().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_ptp5'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=5).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=5).min().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_ptp10'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=10).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=10).min().reset_index(level=(0,1), drop=True).sort_index()
    df['urgency_2_rolling_ptp15'] = df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=15).max().reset_index(level=(0,1), drop=True).sort_index() -  df.groupby(['stock_id',"date_id"])['urgency_2'].rolling(window=15).min().reset_index(level=(0,1), drop=True).sort_index()
...
def generate_all_features(df):
    ...
    feature_name = [i for i in df.columns if i not in ['min_spread_depth_ratio_t',"imbalance_buy_sell_flag","stock_weights","row_id", "target", "time_id", "date_id","far_price_diff_2","matched_imbalance_ret_2","matched_imbalance_ret_3","bid_size_ask_size_imbalance_size_imb2","imbalance_buy_sell_flag_shift_5"]]
...
if is_train:
    ...
    for nl in [1200]:
        for ss in [0.55]:
            for cb in [0.42]:
                ...
                os.system('mkdir models_xgb'+str(len(feature_name)))
                joblib.dump(lgb_model, './models_xgb'+str(len(feature_name))+'/lgbPart.model')
    ...
    os.system('mkdir models_xgb'+str(len(feature_name)))
    joblib.dump(infer_lgb_model, './models_xgb'+str(len(feature_name))+'/lgbFull.model')
    if is_offline:   
        # offline predictions
